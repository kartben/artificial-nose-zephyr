...
static int grove_multichannel_gas_v2_read_gas_sensor(const struct i2c_dt_spec *i2c,
						     uint8_t cmd,
						     uint32_t *res) {
	if (i2c_reg_write_byte_dt(i2c, 0, cmd) < 0) {
		LOG_ERR("Failed to write read command");
		return -EIO;
	}
	k_sleep(K_MSEC(1));

	if (i2c_read_dt(i2c, res, sizeof(res)) < 0) {
		LOG_ERR("Failed to read gas sensor result");
		return -EIO;
	}
	k_sleep(K_MSEC(1));
						 ...
						 struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_VOC) {
		...
		val->val2 = 0;
	} else if (chan == SENSOR_CHAN_CO) {
		val->val1 = drv_data->co;
		val->val2 = 0;
	} else if (chan == SENSOR_CHAN_NO2) {
		val->val1 = drv_data->no2;
		val->val2 = 0;
	} else if (chan == SENSOR_CHAN_C2H5OH) {
